...
// Default cities to fallback to when geolocation fails and no favorites exist
const DEFAULT_CITIES = [
  { name: 'New York', city: 'New York', country: 'US', coordinates: { lat: 40.7128, lon: -74.0060 } },
  { name: 'London', city: 'London', country: 'GB', coordinates: { lat: 51.5074, lon: -0.1278 } },
  { name: 'Tokyo', city: 'Tokyo', country: 'JP', coordinates: { lat: 35.6762, lon: 139.6503 } },
  { name: 'Paris', city: 'Paris', country: 'FR', coordinates: { lat: 48.8566, lon: 2.3522 } },
  { name: 'Sydney', city: 'Sydney', country: 'AU', coordinates: { lat: -33.8688, lon: 151.2093 } },
  { name: 'Toronto', city: 'Toronto', country: 'CA', coordinates: { lat: 43.6532, lon: -79.3832 } },
  { name: 'Mumbai', city: 'Mumbai', country: 'IN', coordinates: { lat: 19.0760, lon: 72.8777 } },
  { name: 'Berlin', city: 'Berlin', country: 'DE', coordinates: { lat: 52.5200, lon: 13.4050 } },
];
...
export const WeatherProvider = ({ children }) => {
  ...
  // Location state
  const { location: currentLocation, error: locationError, isLoading: locationLoading } = useCurrentLocation();
  ...
  useEffect(() => {
    ...
    queryClient.invalidateQueries({ queryKey: ['weather'], refetchType: 'active' });
  }, [preferences.units]);
  ...
  }, [locationLoading, hasInitialized, currentLocation, preferences.autoLocation, selectedLocation, favorites]);
  ...
  const addFavorite = (location) => {
    ...
    if (!location || !cityName) {
      console.error('Cannot add favorite: location or city/name is missing', location);
      return null;
    }
    ...
    setFavorites(prev => {
      ...
      const exists = prev.some(fav => {
        ...
        return favCity?.toLowerCase() === cityName?.toLowerCase() &&
  ...
  const reorderFavorites = ({ sourceIndex, destinationIndex, sourceId, destinationId }) => {
    setFavorites(prev => {
      if (!Array.isArray(prev) || prev.length < 2) return prev;

      let from = typeof sourceIndex === 'number' ? sourceIndex : prev.findIndex(f => f.id === sourceId);
      let to = typeof destinationIndex === 'number' ? destinationIndex : prev.findIndex(f => f.id === destinationId);
      ...
      try {
        ...
        if (!Number.isNaN(idx)) {
          ...
          if (clamped !== idx) localStorage.setItem(rotationIndexKey, String(clamped));
        }
      } catch (_) {
        // non-fatal
      }
  ...
  // Function to get the effective location to use for weather fetching
  ...
  const getRandomDefaultCity = (excludeLocation = null) => {
    try {
      const excludeName = (excludeLocation?.city || excludeLocation?.name || '').toLowerCase();
      const pool = DEFAULT_CITIES.filter(c => (c.city || c.name).toLowerCase() !== excludeName);
      const list = pool.length > 0 ? pool : DEFAULT_CITIES;
  ...
  const value = {
    ...
    formatTemperature: (temp) => {
      return `${Math.round(temp)}${value.getTemperatureUnit()}`;
    },
    
    formatWindSpeed: (speed) => {
      return `${Math.round(speed)} ${value.getWindSpeedUnit()}`;
    },
  };
