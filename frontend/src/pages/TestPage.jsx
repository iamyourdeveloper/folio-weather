...
const TestPage = () => {
  ...
  // Weather Data Tests
  const weatherTest = useCurrentWeatherByCity(testCity, 'metric', { 
  ...
  const ResultDisplay = ({ result, title }) => {
    ...
    return (
      <div>
        <h4>{title} - <StatusBadge status="success">SUCCESS</StatusBadge></h4>
        <pre style={{
          background: '#f1f1f1',
          padding: '10px',
          borderRadius: '4px',
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <p>This page tests the React frontend integration with the backend API using React Query hooks.</p>

      <TestSection title="🔗 Basic API Connection Tests">
        <div style={{ marginBottom: '15px' }}>
          <h4>API Connection Test - <StatusBadge status={apiTest.isLoading ? 'loading' : apiTest.isError ? 'error' : 'success'}>
            {apiTest.isLoading ? 'LOADING' : apiTest.isError ? 'ERROR' : 'SUCCESS'}
          </StatusBadge></h4>
        ...
        <div>
          <h4>Units Test - <StatusBadge status={unitsTest.isLoading ? 'loading' : unitsTest.isError ? 'error' : 'success'}>
            {unitsTest.isLoading ? 'LOADING' : unitsTest.isError ? 'ERROR' : 'SUCCESS'}
          </StatusBadge></h4>
            ...
            <div style={{ background: '#e8f5e8', padding: '10px', borderRadius: '4px' }}>
              ...
              <ul>
                {unitsTest.data.data.units.map(unit => (
                  <li key={unit.key}>{unit.name} ({unit.symbol}) - {unit.description}</li>
                ))}
              </ul>
            </div>
          )}
      ...
      <TestSection title="🌍 Weather Data Tests">
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <p style={{ fontSize: '14px', color: '#666', fontStyle: 'italic' }}>
            💡 Weather and Forecast tests run automatically on page load. Use buttons above to start/stop tests.
          </p>
        </div>
      ...
      <TestSection title="📊 Integration Summary">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
          <div style={{ textAlign: 'center', padding: '10px', background: apiTest.isSuccess ? '#d4edda' : '#f8d7da', borderRadius: '4px' }}>
          ...
          <div style={{ textAlign: 'center', padding: '10px', background: unitsTest.isSuccess ? '#d4edda' : '#f8d7da', borderRadius: '4px' }}>
            <strong>Units Endpoint</strong><br/>
            <StatusBadge status={unitsTest.isSuccess ? 'success' : 'error'}>
              {unitsTest.isSuccess ? 'PASS' : 'FAIL'}
            </StatusBadge>
          </div>
          
          <div style={{ textAlign: 'center', padding: '10px', background: !enableWeatherTest ? '#fff3cd' : weatherTest.isSuccess ? '#d4edda' : '#f8d7da', borderRadius: '4px' }}>
            <strong>Weather Data</strong><br/>
            <StatusBadge status={!enableWeatherTest ? 'idle' : weatherTest.isSuccess ? 'success' : 'error'}>
              {!enableWeatherTest ? 'NOT TESTED' : weatherTest.isSuccess ? 'PASS' : 'FAIL'}
            </StatusBadge>
          </div>
          
          <div style={{ textAlign: 'center', padding: '10px', background: !enableForecastTest ? '#fff3cd' : forecastTest.isSuccess ? '#d4edda' : '#f8d7da', borderRadius: '4px' }}>
            <strong>Forecast Data</strong><br/>
            <StatusBadge status={!enableForecastTest ? 'idle' : forecastTest.isSuccess ? 'success' : 'error'}>
              {!enableForecastTest ? 'NOT TESTED' : forecastTest.isSuccess ? 'PASS' : 'FAIL'}
            </StatusBadge>
          </div>
        </div>
      </TestSection>
      ...
      <TestSection title="ℹ️ Debug Information">
        <div style={{ fontSize: '14px', color: '#666' }}>
          ...
          <p><strong>React Query Dev Tools:</strong> Check browser console and network tab</p>
        </div>
      </TestSection>
    </div>
  );
