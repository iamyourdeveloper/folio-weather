/**
 * Complete US Cities Database - All 50 States + DC
 * This comprehensive database contains thousands of US cities, towns, villages, 
 * and incorporated places for exhaustive search coverage.
 * 
 * Total: ~15,000+ US locations across all states
 */
...
export const ALL_US_CITIES_COMPLETE = {
  ...
  'AK': [
    ...
    'Nome', 'Kotzebue', 'Dillingham', 'Valdez', 'Cordova', 'Wrangell', 'Petersburg',
    'Craig', 'Skagway', 'Delta Junction', 'Tok', 'Glennallen', 'Soldotna', 'Sterling',
    'Ninilchik', 'Talkeetna', 'Healy', 'Nenana', 'Fort Yukon', 'Galena', 'McGrath',
    'Aniak', 'Emmonak', 'Mountain Village', 'Saint Marys', 'Alakanuk', 'Pilot Station'
  ],
  ...
  'AR': [
    ...
    'El Dorado', 'England', 'Eureka Springs', 'Farmington', 'Forrest City', 'Gentry'
  ],
  ...
  'CO': [
    ...
    'Alamosa', 'Aspen', 'Avon', 'Basalt', 'Black Hawk', 'Breckenridge', 'Brush',
    'Buena Vista', 'Burlington', 'Canon City', 'Carbondale', 'Central City', 'Craig',
    'Crested Butte', 'Cripple Creek', 'Deer Trail', 'Delta', 'Durango', 'Eagle',
    'Eaton', 'Edgewater', 'Elizabeth', 'Erie', 'Estes Park', 'Evans', 'Federal Heights'
  ],
  ...
  'DE': [
    ...
    'Rehoboth Beach', 'Bethany Beach', 'Lewes', 'Fenwick Island', 'Dewey Beach',
    'Arden', 'Ardencroft', 'Ardentown', 'Bellefonte', 'Bethel', 'Blades', 'Bowers',
    'Bridgeville', 'Camden', 'Cheswold', 'Claymont', 'Clayton', 'Dagsboro', 'Delaware City'
  ],
  ...
  'GA': [
    ...
    'Acworth', 'Adairsville', 'Adel', 'Alma', 'Americus', 'Ashburn', 'Bainbridge',
    'Ball Ground', 'Barnesville', 'Baxley', 'Blackshear', 'Blairsville', 'Blue Ridge'
  ],
  ...
  'HI': [
    ...
    'Ahuimanu', 'Captain Cook', 'Diamond Head', 'Eleele', 'Haiku', 'Hanamaulu',
    'Hauula', 'Hawaiian Beaches', 'Hawaiian Ocean View', 'Hickam Field', 'Holualoa'
  ],
  ...
  'ID': [
    ...
    'Aberdeen', 'American Falls', 'Arco', 'Ashton', 'Athol', 'Bellevue', 'Blackfoot',
    'Bliss', 'Bonners Ferry', 'Buhl', 'Cambridge', 'Cascade', 'Challis', 'Council'
  ],
  ...
  'IN': [
    ...
    'Alexandria', 'Anderson', 'Angola', 'Auburn', 'Batesville', 'Bedford', 'Beech Grove'
  ],
  ...
  'IA': [
    ...
    'Adel', 'Algona', 'Altoona', 'Anamosa', 'Atlantic', 'Boone', 'Carroll', 'Centerville'
  ],
  ...
  'KS': [
    ...
    'Abilene', 'Andover', 'Arkansas City', 'Atchison', 'Augusta', 'Baldwin City', 'Basehor'
  ],
  ...
  'KY': [
    ...
    'Ashland', 'Murray', 'St. Matthews', 'Alexandria', 'Bardstown', 'Berea', 'Campbellsville'
  ],
  ...
  'LA': [
    ...
    'Abbeville', 'Addis', 'Baker', 'Bastrop', 'Bogalusa', 'Breaux Bridge', 'Broussard'
  ],
  ...
  'ME': [
    ...
    'Bar Harbor', 'Bath', 'Belfast', 'Berwick', 'Brewer', 'Bucksport', 'Calais'
  ],
  ...
  'VT': [
    ...
    'Essex', 'Bennington', 'Milton', 'Williston', 'Middlebury', 'St. Johnsbury'
  ],
...
/**
 * Enhanced search function for US cities with fuzzy matching and state search
 * @param {string} query - Search query
 * @param {number} limit - Maximum results to return (default: 50)
 * @returns {Array} Array of matching cities with state information
 */
export const searchUSCities = (query, limit = 50) => {
  if (!query || typeof query !== 'string') return [];
  
  const normalizedQuery = query.toLowerCase().trim();
  const results = [];
  
  // Check if query is a state name or abbreviation
  const stateResults = searchByState(normalizedQuery);
  if (stateResults.length > 0) {
    // If searching for a state, return major cities from that state
    results.push(...stateResults.slice(0, Math.min(limit, 20)));
  }
  ...
  Object.keys(US_CITY_SEARCH_INDEX).forEach(cityKey => {
    if (cityKey.startsWith(normalizedQuery) && cityKey !== normalizedQuery) {
  ...
  if (results.length < limit) {
    Object.keys(US_CITY_SEARCH_INDEX).forEach(cityKey => {
      if (cityKey.includes(normalizedQuery) && 
          !cityKey.startsWith(normalizedQuery) && 
          cityKey !== normalizedQuery) {
        results.push(...US_CITY_SEARCH_INDEX[cityKey]);
      }
    });
  }
...
/**
 * Search for cities by state name or abbreviation
 * @param {string} stateQuery - State name or abbreviation
 * @returns {Array} Array of cities in the matching state
 */
export const searchByState = (stateQuery) => {
  if (!stateQuery || typeof stateQuery !== 'string') return [];
  
  const normalizedStateQuery = stateQuery.toLowerCase().trim();
  ...
  let stateCode = stateNames[normalizedStateQuery];
  
  // Check if it's already a state abbreviation
  if (!stateCode) {
    const upperQuery = normalizedStateQuery.toUpperCase();
    if (Object.values(stateNames).includes(upperQuery)) {
 ...
 * @param {number} count - Number of random cities to return
 * @returns {Array} Array of random cities
 */
export const getRandomUSCities = (count = 10) => {
  const shuffled = ALL_US_CITIES_FLAT.sort(() => 0.5 - Math.random());
  return shuffled.slice(0, count);
};

export default ALL_US_CITIES_COMPLETE;
