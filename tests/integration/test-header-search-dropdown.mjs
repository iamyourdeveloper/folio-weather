...
async function testAPI(endpoint, description) {
  try {
    log("blue", `Testing: ${description}`);
    const response = await fetch(`http://localhost:8000${endpoint}`);
    const data = await response.text();

    if (response.ok) {
      log("green", `✓ ${description} - Status: ${response.status}`);
      return true;
    } else {
      log("red", `✗ ${description} - Status: ${response.status}`);
      return false;
    }
  } catch (error) {
  ...
  try {
    ...
    const integrationChecks = [
      ...
      },
      {
        ...
        pattern: /HeaderSearchDropdown.*onSelect={handleDropdownSelect}/,
      },
      {
        name: "Mobile dropdown integration",
        pattern: /HeaderSearchDropdown.*onSelect={handleMobileDropdownSelect}/,
      },
      {
        name: "Props configuration",
        pattern: /maxSuggestions.*minQueryLength.*debounceMs/,
      },
    ];
...
async function runTests() {
  ...
  log("yellow", "\n4️⃣  Testing Backend APIs...");
  const searchApiOk = await testAPI(
    "/api/search/suggestions?q=London&limit=5",
    "Search API"
  );
  const weatherApiOk = await testAPI(
    "/api/weather/current/city/London?units=metric",
    "Weather API"
  );

  const apiOk = searchApiOk && weatherApiOk;
  ...
  log(apiOk ? "green" : "red", `APIs: ${apiOk ? "PASS" : "FAIL"}`);

  const allPassed = componentOk && integrationOk && stylesOk && apiOk;
